...
public class GTimer {
	...
	public void startTimer(Label time, long startTime) { // TODO: EDIT HERE
		...
		TimerTask timerTask = new TimerTask() {
			...
			public void run() {
				...
				if (timing == true) {
					...
					// Finds difference between current time and the previous
					// time and adds it to the running total time
					currentTimeTaken += (currentTime.getTime() - prevTime.getTime());
					prevTime = currentTime;// Sets the current time to the
											// previous time
					...
					// Tells javafx thread to update the label with the new
					// output ASAP
		...
		// Starts the timer.
		timer.scheduleAtFixedRate(timerTask, 0, 100);// this line starts the
														// timer at the same
														// time its executed
	}
