...
public class LevelSelectorScene {
	...
	private void setLevelSelect(MouseEvent e) {
		try {
			Level currentLevel;
			if (LevelManager.hasUnfinishedLevel(currentLevelIndex, this.currentProfile.getId())) {
				currentLevel = LevelManager.resume(currentLevelIndex, this.currentProfile.getId());
			} else {
				currentLevel = LevelManager.load(currentLevelIndex);
				// currentLevel = LevelManager.load(LevelManager.load().get(currentLevelIndex - 1));
			}
	...
	private void setLevelNum(boolean prevNext) {
		if (prevNext && currentLevelIndex < maxLevel) {
			...
			if (!prevNext && currentLevelIndex > 1) {
				levelSelectorController.getLevelNum().setText("Level " + (--currentLevelIndex).toString());// This
																											// could
																											// be
																											// used
																											// for
