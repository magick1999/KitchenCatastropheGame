...
public class Player extends MovableObject {
	...
	public void move() {
		...
		if (nextCell != null) {
			...
			if (collisionResult.isColliding()) {
				...
				this.onCellStepped(nextCell); // Does nothing for Teleporters =>
												// safe
			}
		}
	}
	...
	private void onCellStepped(StepableCell cell) {
		if (cell instanceof Ground) {
			...
			if (ground.hasCollectableItem()) {
				CollectableItem item = ground.collect(); // Collect the
															// CollectableItem
															// if the is any
				if (item instanceof Token) {
					this.getTokenAccumulator().addToken((Token) item); // If
																		// token,
																		// add
																		// to
																		// TokenAccumulator
	...
	public String toString() {
		...
		builder.append(String.format(TO_STRING_PATTERN, Constants.TYPE_PLAYER, this.getTitle(), this.getPositionX(),
