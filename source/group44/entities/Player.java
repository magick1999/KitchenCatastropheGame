...
public class Player extends MoveableObject {
    private Node player;
    private ArrayList<CollectibleObject> inventory;

    public void useCollectibleItem(CollectibleObject item, LevelObject obj) {
        for (CollectibleObject anObj : inventory) {
            if (item == anObj) {
                inventory.remove(anObj);
                // obj.interact(item); //this method has to be written or some further
                // development as to how they will interact.
            }
        }
    }
    ...
    private void movePlayerTo(double x, double y) {
        ...
        if (x - cx >= 0 && x + cx <= Main.getWidth() && y - cy >= 0 && y + cy <= Main.getHeight()) {
