...
public class LevelTest {
    ...
    private static String PATH_IMAGE_GROUND = BASE_PATH_IMAGE
	    + "cells/floor.png";
    private static String PATH_IMAGE_WATER = BASE_PATH_IMAGE
	    + "cells/water.png";
    ...
    private static String PATH_IMAGE_TELEPORTER = BASE_PATH_IMAGE
	    + "cells/teleporter.png";

    private static String PATH_IMAGE_DOOR_KEY_BLUE = BASE_PATH_IMAGE
	    + "cells/blueDoor.png";
    private static String PATH_IMAGE_DOOR_KEY_GOLD = BASE_PATH_IMAGE
	    + "cells/goldDoor.png";
    private static String PATH_IMAGE_DOOR_KEY_GREEN = BASE_PATH_IMAGE
	    + "cells/greenDoor.png";
    private static String PATH_IMAGE_DOOR_KEY_RED = BASE_PATH_IMAGE
	    + "cells/redDoor.png";

    private static String PATH_IMAGE_DOOR_TOKEN = BASE_PATH_IMAGE
	    + "cells/tokenDoor.png";

    private static String PATH_IMAGE_COLLECATBLE_FLIPPERS = BASE_PATH_IMAGE
	    + "cells/flippers.png";
    private static String PATH_IMAGE_COLLECATBLE_FIRE_BOOTS = BASE_PATH_IMAGE
	    + "cells/fireBoots.png";
    private static String PATH_IMAGE_COLLECATBLE_TOKEN = BASE_PATH_IMAGE
	    + "cells/token.png";

    private static String PATH_IMAGE_PLAYER = BASE_PATH_IMAGE
	    + "ChefDownWalk/Front1.png";

    private static String PATH_IMAGE_SMART_TARGETING_ENEMY = BASE_PATH_IMAGE
	    + "Enemies/Egg/mrEggFront.png";
    private static String PATH_IMAGE_WALL_FOLLOWING_ENEMY = BASE_PATH_IMAGE
	    + "Enemies/Hotdog/mrHotDogFront.png";
    private static String PATH_IMAGE_STRAIGHT_WALKING_ENEMY = BASE_PATH_IMAGE
	    + "Enemies/Carrot/mrCarrotFront.png";
    private static String PATH_IMAGE_DUMB_TARGETTING_ENEMY = BASE_PATH_IMAGE
	    + "Enemies/Pickle/mrPickleFront.png";
	...
	if (true) {
		...
		} catch (FileNotFoundException | CollisionException
			| ParsingException e) {
    ...
    private static void generateBorders(PrintWriter writer, int width,
	    int height) {
	// TOP BORDER
    ...
    public static void generateLevelFile01(String path, int width, int height,
	    int time) throws FileNotFoundException {
	int id = 1;

	PrintWriter writer = new PrintWriter(path);

	writer.println(
		String.format(FILE_HEADER_PATTERN, id, width, height, time));
    ...
    public static void generateLevelFile02(String path, int time)
	...
	for (int x = 1; x < WIDTH - 1; x++) {
		...
		if (x == 1 && y == 9) {
		    printBlueKey(writer, x, y); // Ground +
						// BlueKey
		} else if (x == 1 && y == 18) {
		    printPlayer(writer, x, y); // Ground +
					       // Player
		} else if (x == 3 && y == 7) {
		    printGoldKey(writer, x, y); // Ground +
						// BlueKey
		} else if (x == 4 && y == 9) {
		    printBlueKeyDoor(writer, x, y); // Blue
						    // Key
						    // Door
		} else if ((x == 4 || x == 6) && y != 9) {
		    ...
		    printGoldKeyDoor(writer, x, y); // Gold
						    // Key
						    // Door
		} else if (x == 5 && y != 9) {
    ...
    public static void generateLevelFile03(String path, int time)
	    throws FileNotFoundException {
	int id = 3;
    ...
    public static void generateLevelFile04(String path, int time)
	    throws FileNotFoundException {
	int id = 4;
	PrintWriter writer = new PrintWriter(path);
	...
	writer.print(String.format(PARSE_PATTERN_CELL, Constants.TYPE_WALL, x,
		y, PATH_IMAGE_WALL));
    }

    // Steppable cells
    private static void printGround(PrintWriter writer, int x, int y) {
	writer.print(String.format(PARSE_PATTERN_CELL, Constants.TYPE_GROUND, x,
		y, PATH_IMAGE_GROUND));
    }

    private static void printWater(PrintWriter writer, int x, int y) {
	writer.print(String.format(PARSE_PATTERN_CELL, Constants.TYPE_WATER, x,
		y, PATH_IMAGE_WATER));
    }

    private static void printFire(PrintWriter writer, int x, int y) {
	writer.print(String.format(PARSE_PATTERN_CELL, Constants.TYPE_FIRE, x,
		y, PATH_IMAGE_FIRE));
    }

    private static void printGoal(PrintWriter writer, int x, int y) {
	writer.print(String.format(PARSE_PATTERN_CELL, Constants.TYPE_GOAL, x,
		y, PATH_IMAGE_GOAL));
    }

    private static void printBlueKeyDoor(PrintWriter writer, int x, int y) {
	writer.print(String.format(PARSE_PATTERN_DOOR, Constants.TYPE_KEY_DOOR,
		DOOR_KEY_BLUE, x, y, PATH_IMAGE_DOOR_KEY_BLUE,
		PATH_IMAGE_GROUND, KeyType.BLUE.getKeyCode()));
    }

    private static void printGoldKeyDoor(PrintWriter writer, int x, int y) {
	writer.print(String.format(PARSE_PATTERN_DOOR, Constants.TYPE_KEY_DOOR,
		DOOR_KEY_GOLD, x, y, PATH_IMAGE_DOOR_KEY_GOLD,
		PATH_IMAGE_GROUND, KeyType.GOLD.getKeyCode()));
    }

    private static void printGreenKeyDoor(PrintWriter writer, int x, int y) {
	writer.print(String.format(PARSE_PATTERN_DOOR, Constants.TYPE_KEY_DOOR,
		DOOR_KEY_GREEN, x, y, PATH_IMAGE_DOOR_KEY_GREEN,
		PATH_IMAGE_GROUND, KeyType.GREEN.getKeyCode()));
    }

    private static void printRedKeyDoor(PrintWriter writer, int x, int y) {
	writer.print(String.format(PARSE_PATTERN_DOOR, Constants.TYPE_KEY_DOOR,
		DOOR_KEY_RED, x, y, PATH_IMAGE_DOOR_KEY_RED, PATH_IMAGE_GROUND,
		KeyType.RED.getKeyCode()));
    }

    private static void printTokenDoor(PrintWriter writer, int x, int y,
	    int tokens) {
	writer.print(String.format(PARSE_PATTERN_DOOR,
		Constants.TYPE_TOKEN_DOOR, DOOR_KEY_TOKEN, x, y,
		PATH_IMAGE_DOOR_TOKEN, PATH_IMAGE_GROUND, tokens));
    }

    private static void printTeleporter(PrintWriter writer, int x, int y) {
	writer.print(String.format(PARSE_PATTERN_CELL_TITLE,
		Constants.TYPE_TELEPORTER, TELEPORTER_NAME, x, y,
	...
	printGround(writer, x, y);
	writer.print(String.format(PARSE_PATTERN_COLLECTABLE_KEY,
		Constants.TYPE_KEY, KeyType.BLUE.getKeyCode()));
    }

    private static void printGoldKey(PrintWriter writer, int x, int y) {
	printGround(writer, x, y);
	writer.print(String.format(PARSE_PATTERN_COLLECTABLE_KEY,
		Constants.TYPE_KEY, KeyType.GOLD.getKeyCode()));
    }

    private static void printGreenKey(PrintWriter writer, int x, int y) {
	printGround(writer, x, y);
	writer.print(String.format(PARSE_PATTERN_COLLECTABLE_KEY,
		Constants.TYPE_KEY, KeyType.GREEN.getKeyCode()));
    }

    private static void printRedKey(PrintWriter writer, int x, int y) {
	printGround(writer, x, y);
	writer.print(String.format(PARSE_PATTERN_COLLECTABLE_KEY,
		Constants.TYPE_KEY, KeyType.RED.getKeyCode()));
    }

    private static void printToken(PrintWriter writer, int x, int y) {
	printGround(writer, x, y);
	writer.print(String.format(PARSE_PATTERN_COLLECTABLE_NOTKEYS,
		Constants.TYPE_TOKEN, PATH_IMAGE_COLLECATBLE_TOKEN));
    }
	...
	printGround(writer, x, y);
	writer.print(String.format(PARSE_PATTERN_COLLECTABLE_NOTKEYS,
		Constants.TYPE_FIRE_BOOTS, PATH_IMAGE_COLLECATBLE_FIRE_BOOTS));
    }

    private static void printFlippers(PrintWriter writer, int x, int y) {
	printGround(writer, x, y);
	writer.print(String.format(PARSE_PATTERN_COLLECTABLE_NOTKEYS,
		Constants.TYPE_FLIPPERS, PATH_IMAGE_COLLECATBLE_FLIPPERS));
    }

    // Movable objects
    private static void printWallFollowingEnemy(PrintWriter writer, int x,
	    int y) {
	printGround(writer, x, y);
	writer.print(String.format(PARSE_PATTERN_ENEMY,
		Constants.TYPE_WALL_FOLLOWING_ENEMY, ENEMY_NAME_WALL_FOLLOWING,
		x, y, PATH_IMAGE_WALL_FOLLOWING_ENEMY));
    }

    // ENEMY_NAME_SMART_TARGETING
    private static void printSmartTargetingEnemy(PrintWriter writer, int x,
	    int y) {
	printGround(writer, x, y);
	writer.print(String.format(PARSE_PATTERN_ENEMY,
		Constants.TYPE_SMART_TARGETING_ENEMY,
		ENEMY_NAME_SMART_TARGETING, x, y,
		PATH_IMAGE_SMART_TARGETING_ENEMY));
    }

    private static void printStraightWalkingEnemy(PrintWriter writer, int x,
	    int y, int vx, int vy) {
	// ",%s,%s,%d,%d,%d,%d,%s"
	printGround(writer, x, y);
	writer.print(String.format(PARSE_PATTERN_ENEMY_VELOCITY,
		Constants.TYPE_STRAIGHT_WALKING_ENEMY,
		ENEMY_NAME_STRAIGHT_WALKING, x, y, vx, vy,
		PATH_IMAGE_STRAIGHT_WALKING_ENEMY));
    }

    private static void printDumbTargetingEnemy(PrintWriter writer, int x,
	    int y) {
	printGround(writer, x, y);
	writer.print(String.format(PARSE_PATTERN_ENEMY,
		Constants.TYPE_DUMB_TARGETING_ENEMY, ENEMY_NAME_DUMB_TARGETING,
		x, y, PATH_IMAGE_DUMB_TARGETTING_ENEMY));
    }

    private static void printPlayer(PrintWriter writer, int x, int y) {
	printGround(writer, x, y);
	writer.print(String.format(PARSE_PATTERN_PLAYER, Constants.TYPE_PLAYER,
		PLAYER_NAME, x, y, PLAYER_VECTOR_X, PLAYER_VECTOR_Y,
		PATH_IMAGE_PLAYER));
    }

    // Others
    private static void printTeleportersLink(PrintWriter writer, int x1, int y1,
	    int x2, int y2) {
	writer.print(String.format(PARSE_PATTERN_TELEPORTER_LINK,
		Constants.TYPE_TELEPORTER_LINK, x1, y1, x2, y2));
    }
}
